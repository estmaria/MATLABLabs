...
nVar = 500;            % Number of Decision Variables CAMBIAR DEPENDIENDO DEL RANGE DE LA i
...
% PSO Parameters
w = 1;            % Inertia Weight
wdamp = 0.995;     % Inertia Weight Damping Ratio
...
for it = 1:MaxIt
    ...
    for i = 1:nPop
        ...
        % Update Personal Best
        if particle(i).Cost>particle(i).Best.Cost  %flip to find max  max(>) min(<)
            
            particle(i).Best.Position = particle(i).Position;
            particle(i).Best.Cost = particle(i).Cost;
            
            % Update Global Best
            if particle(i).Best.Cost>GlobalBest.Cost  %flip to find max  max(>) min(<)
